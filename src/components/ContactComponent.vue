  ...
  <div>
    ...
    <div class="container-fluid worker-page" v-else>
      <div class="row d-card flex-nowrap">
        ...
        <div class="worker-right-col" v-if="user.comment || activeTab === 'settings'">
          <div class="worker-col-inner d-p-20">
    ...
    methods: {
      ...
      recalculatePageTitle() {
        ...
        this.pageData = {
          ...
          metaTitle: this.edit_user.user_id ? 'Профиль контакта' : 'Создание контакта'
        };
      },
      calculateRouteByUsertype(t) {
        if(this.user_id === this.$store.state.user.user_id) { return '/profile'; }
        return this.routesByTypes[t] ? this.routesByTypes[t] : '/crm/contacts/profile';
      },
