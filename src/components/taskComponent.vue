<template>
  <div :class="'overlay-modal-outer '+(allowEditTask ? 'overlay-modal-outer-edit' : '')" :id="id">
    <div class="overlay-modal-backdrop" @click="$router.push(clearLinkMode())"></div>
    <div class="overlay-modal-inner c-scrollbar">
      <div class="overlay-modal-wrap">
        <div class="overlay-modal-left u-mr-small">
          <div class="c-card">
            <div class="overlay-modal-header u-pv-small u-ph-medium">
              <div class="overlay-modal-title">
                <input type="text" v-model="e_task.title" v-if="allowEditTask"  :class="'overlay-modal-title-input '+(e_task.title ? 'blank' : '')"/>
                <div class="overlay-modal-title-input u-border-bottom-zero" v-html="e_task.title" v-else ></div>
                <div class="overlay-modal-header-icons">
                  <div @click="set('is_important', !e_task.is_important)" title="Горит">
                    <svg class="overlay-modal-header-icon overlay-modal-important-icon" fill="none"  viewBox="0 0 14 18"  xmlns="http://www.w3.org/2000/svg" v-if="!e_task.is_important">
                    ...
                    <svg  class="overlay-modal-header-icon overlay-modal-important-icon"  viewBox="0 0 14 18" fill="none" xmlns="http://www.w3.org/2000/svg" v-else>
                  ...
                  <div @click="set('is_favourite', !e_task.is_favourite)">
                    <svg :class="'overlay-modal-header-icon overlay-modal-favourite-icon ' +(e_task.is_favourite ? 'active' : '')" title="В закладки" viewBox="0 0 18 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <div title="Добавить теги"  @click="toggleTagsDropdown($event)">
                    <svg  :class="'overlay-modal-header-icon overlay-modal-tags-icon '+(tagsAreShown ? 'active' : '')" viewBox="0 0 23 21" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <div class="overlay-modal-subtitle-bar">
                <div class="overlay-modal-tags" v-if="e_task.tags.length">

                  <div class="trigger overlay-modal-tag" v-for="tag, i in e_task.tags" @click="e_task.tags.splice(i, 1)">
                    <div class="overlay-modal-tag-icon"><i :class="'fa fa-circle-o u-color-'+getTagCls(tag)"></i></div>
                    <div class="" v-html="getTagTitle(tag)"></div>
                  </div>
                </div>
                <div class="overlay-modal-subtitle-tools">

                </div>
              </div>
              <div class="overlay-modal-target">
              ...
              <div class="overlay-modal-editor">
                <div class="overlay-modal-editor-inner">
                  <div
                    :class="'hidden-editr-toolbar ' +(!taskEditorToolbar ? 'no-toolbar' : '')+ (e_task.checklist.length ? ' overlay-modal-editor-w-checklist' : '')">
                    <wysiwyg v-model="e_task.description" placeholder=""/>
                  </div>
                  <div class="overlay-modal-content-icons">
                    <div class="overlay-modal-header-icons">
                      <svg class="overlay-modal-header-icon overlay-modal-header-icon-right" @click="taskEditorToolbar = !taskEditorToolbar" viewBox="0 0 21 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="overlay-modal-content-icons u-mt-xsmall">
                  <div class="overlay-modal-header-icons">
                    <svg xmlns="http://www.w3.org/2000/svg" title="Добавить пункт чек-листа" class="overlay-modal-header-icon" @click="checklistTab = !checklistTab" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px"  viewBox="0 0 594.149 594.149" style="enable-background:new 0 0 594.149 594.149;" xml:space="preserve">
...
<svg title="Добавить файл" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"  x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve" class="overlay-modal-header-icon overlay-modal-files-ico" @click="selectTaskFile()">
        ...
        <div class="overlay-modal-right c-card u-pv-small u-ph-medium">
          ...
          <div class="overlay-modal-row" v-if="0">
            <div class="overlay-modal-k">Ответственный:</div>
            <div class="overlay-modal-v">
              <div class="u-inline-block overlay-modal-tags-users-wrapper">

                <!-- 'renderUserType', 'renderUserAvatar', 'user', 'title', 'action', 'selectTitle', 'renderUserRealName' -->
              </div>
              <div :class="'overlay-modal-additional-toggler '+(acceptingUserEl ? 'active' : '')" @click="toggleEl('acceptingUser')">Принимающий</div>
              <div :class="'overlay-modal-additional-toggler '+(initiatingUserEl ? 'active' : '')" @click="toggleEl('initiatingUser')">Инициатор</div>
              <div :class="'overlay-modal-additional-toggler '+(coperformersUsersEl ? 'active' : '')" @click="toggleEl('coperformersUsers')">Соисполнители</div>
              <div :class="'overlay-modal-additional-toggler '+(observersUsersEl ? 'active' : '')" @click="toggleEl('observersUsers')">Наблюдатели</div>
            </div>
          </div> <!-- end resp -->
          <div class="overlay-modal-tab-content" v-show="activeTab === 'details'" :key="'overlay-modal-tab-details'" v-if="0">

            <div class="u-bg-white overlay-modal-padding u-border-top u-border-bottom mh-200  u-width-100">

              <!--

              -->
              <div class="overlay-modal-row overlay-modal-row-togglable" id="acceptingUser" >
                <div class="overlay-modal-k">Принимающий:</div>
                <div class="overlay-modal-v">
                  <div class="u-inline-block overlay-modal-tags-users-wrapper">
              ...
              <div class="overlay-modal-row overlay-modal-row-togglable" id="initiatingUser">
                <div class="overlay-modal-k">Инициировавший:</div>
                <div class="overlay-modal-v">
                  <div class="u-inline-block overlay-modal-tags-users-wrapper">
              ...
              <div class="overlay-modal-row overlay-modal-row-togglable" id="coperformersUsers">
                <div class="overlay-modal-k">Соисполнители:</div>
                <div class="overlay-modal-v">
                  <div class="">
              ...
              <div class="overlay-modal-row overlay-modal-row-togglable" id="observersUsers">
                <div class="overlay-modal-k">Наблюдатели:</div>
                <div class="overlay-modal-v">
                  <div class="">
    ...
    <div class="overlay-modal-buttons u-bg-white overlay-modal-padding u-border-top u-width-100" style="display: none;">
  ...
  export default {
    ...
    methods: {
      ...
      confirmSelectTaskFile() {
        ...
        $http.post('/api/files/upload-task-file', formData, {
          headers: {
      ...
      toggleModal(v) {
        let el = $('#' + this.id);
        let el2 = el.find('.overlay-modal-backdrop');
        if (v) {
          el.show();
          el2.stop().fadeIn(300);
          el.addClass('show');
